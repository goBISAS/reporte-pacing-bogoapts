...
# ==========================================================
# 2. MENÚ LATERAL PRINCIPAL (BLINDADO)
# ==========================================================
with st.sidebar:
    try:
    ...
    # Rutas lógicas sin emojis para evitar errores de copia
    opciones_menu = {
...
if vista_seleccionada == "Pacing":
    ...
    with st.sidebar:
        mes_seleccionado = st.selectbox(
            "📅 Seleccione el Mes de Reporte:", 
            options=meses_disponibles
        )

    st.title(f"🏢 Sistema Inteligente BogoApts: {mes_seleccionado.title()}")

    b_url = "https://docs.google.com/spreadsheets/d/"
    id_pac = "1Qkw-Fi3tLvY68maHxJOmHlX9sx0kOvNg-150YRE42W0"
    url_base_pacing = b_url + id_pac + "/"
    ...
    try:
        ...
        for idx, row in df_datos_pacing.iterrows():
            ...
            if celda_camp == '' or any(k in celda_camp.lower() for k in ['campaign', 'campaña', 'nombre', 'total']):
                continue
    ...
    if pacing_exitoso:
        ...
        st.success(f"✅ Sincronización exitosa con [{mes_seleccionado}] | Registro: {fecha_update}")
        st.divider()
        ...
        if not df_plot.empty:
            fig = px.treemap(
                df_plot, path=['Medio_Labels', 'Objetivo'], values='Gasto', color='Gasto', 
        ...
        st.error("No se pudieron cargar los datos operativos de la pauta.")
...
# ==========================================================
    ...
    r_base = "https://docs.google.com/spreadsheets/d/"
    r_id = "190FjfTc6ZsAsRsj3swki1Ch6BME6j2CbfgyxcUt1pMY"
    r_end = "/gviz/tq?tqx=out:csv&gid=212931455"
    url_roas = r_base + r_id + r_end
    ...
    if roas_ok and not df_hist.empty:
        ...
        if df_filtrado.empty:
            ...
            with col1:
                ...
                fig_evolucion.add_trace(go.Bar(
                    x=df_filtrado['Periodo'], y=df_filtrado['Ventas Atribuidas'], 
                    name='Ventas', marker_color='#d6b58e', text=text_v, textposition='outside'
                ))
                fig_evolucion.add_trace(go.Bar(
                    x=df_filtrado['Periodo'], y=df_filtrado['Inversión Total'], 
                    name='Spend', marker_color='#5b3f8e', text=text_i, textposition='outside'
                ))
                fig_evolucion.update_layout(
                    barmode='group', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                    font_color="white", margin=dict(t=20, b=20, l=10, r=10),
            ...
            with col2:
                ...
                if total_l > 0:
                    v_txt = format_label(total_ventas)
                    txt_embudo = [str(total_l), str(total_c), f"{total_ci} Unds<br>({v_txt})"]
                    
                    fig_funnel = go.Figure(go.Funnel(
                        y=["Leads Atribuidos", "Cotizaciones", "Cierres Efectivos"],
                        x=[total_l, total_c, total_ci], text=txt_embudo, textinfo="text+percent initial",
                    ...
                    st.info("No hay suficientes registros para el embudo en este periodo.")
        ...
        st.info("No se detectaron datos válidos. Verifica la conexión con el Google Sheet.")
